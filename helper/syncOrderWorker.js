...
const upsertCustomerFromOrder = async (order, organizationId, storeId, userId) => {
  try {
    ...
    if (!order.billing || !order.billing.email) {
      console.log(`⚠️ Order ${order.id} has no billing email, skipping customer extraction`);
      return null;
    }
    ...
    if (existingCustomer) {
      ...
      // Create new customer - local_id will be auto-generated by the schema default
      console.log(`➕ Creating new customer: ${email}`);
      const newCustomer = new Customer(customerData);
      await newCustomer.save();
      console.log(`✅ Customer created successfully: ${email} (ID: ${newCustomer._id})`);
      return newCustomer._id;
    }
  } catch (error) {
    console.error(`❌ Failed to upsert customer for order ${order.id}:`, error.message);
    console.error(`❌ Full error:`, error);
    return null; // Return null on error, order can still be created without customer reference
  }
};
    ...
    for (const order of orders) {
      try {
        ...
        if (!existingOrder) {
          ...
          if (deletedOrder) {
            skipped++;
            console.log(`⏭️ Skipping order ${order.id} - previously deleted from dashboard on ${deletedOrder.deletedAt}`);
